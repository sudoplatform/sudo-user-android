 ...
 */
suspend fun <T : Mutation<D>, D : Mutation.Data> SimpleGraphQLRequest<D>.mutate(apiCategory: ApiCategory, parserBuilder: () -> T, variablesSerializer: GraphQLRequest.VariablesSerializer? = null): GraphQLResponse<D> = suspendCoroutine { cont ->
    val actualMutation = SimpleGraphQLRequest<String>(this.query, this.variables, String::class.java, variablesSerializer ?: GsonVariablesSerializer(GsonFactoryExt.instance()))
 ...
 */
suspend fun <T : Query<D>, D : Query.Data> SimpleGraphQLRequest<D>.query(apiCategory: ApiCategory, parserBuilder: () -> T, variablesSerializer: GraphQLRequest.VariablesSerializer? = null): GraphQLResponse<D> = suspendCoroutine { cont ->
    val actualQuery = SimpleGraphQLRequest<String>(this.query, this.variables, String::class.java, variablesSerializer ?: GsonVariablesSerializer(GsonFactoryExt.instance()))
...
(private val consumer: Consumer<GraphQLResponse<D>>) : Consumer<String> {
...
internal class FactoryWrapper : GraphQLResponse.Factory {
    override fun <R : Any?> buildResponse(request: GraphQLRequest<R>?, apiResponseJson: String?): GraphQLResponse<R> {
        throw ApiException("FactoryWrapper.buildResponse unexpectedly called", "Reconsider the way in which this GraphQLOperation is being used.")
    }
}
internal class GraphQLOperationWrapper<T>(private val operation: GraphQLOperation<String>, graphQLRequest: GraphQLRequest<T>) :
...
fun <T : Subscription<D>, D : Subscription.Data> SimpleGraphQLRequest<D>.subscribe(
    ...
    val operation = apiCategory.subscribe(
        actualSubscription,
